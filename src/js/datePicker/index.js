...
export default class datePicker extends EventEmitter {
  ...
  set start(date = undefined) {
    if (date) {
      if (type.isDate(date)) {
        this._date.start = this._isValidDate(date, this.min, this.max)
      ...
      if (type.isString(date)) {
        this._date.start = this._isValidDate(
          ...
          ? dateFns.startOfDay(dateFns.parse(date))
          : this._date.start;
      }
    } else {
  ...
  set end(date = undefined) {
    if (date) {
      if (type.isDate(date)) {
        this._date.end = this._isValidDate(date, this.min, this.max)
      ...
      if (type.isString(date)) {
        this._date.end = this._isValidDate(
          ...
          ? dateFns.endOfDay(dateFns.parse(date))
          : this._date.end;
      }
    } else {
  ...
  set min(date = undefined) {
    if (date) {
      if (type.isDate(date)) {
        this._min = this._isValidDate(date)
          ? dateFns.startOfDay(date)
          : this._min;
      }
      if (type.isString(date)) {
        this._min = this._isValidDate(dateFns.parse(date))
          ? dateFns.startOfDay(date)
          : this._min;
      }
    }
  ...
  set max(date = null) {
    if (date) {
      if (type.isDate(date)) {
        this._max = this._isValidDate(date)
          ? dateFns.endOfDay(date)
          : this._max;
      }
      if (type.isString(date)) {
        this._max = this._isValidDate(dateFns.parse(date))
          ? dateFns.endOfDay(date)
          : this._max;
      }
    }
  ...
  onPreviousDatePicker(e) {
    ...
    const prevMonth = dateFns.lastDayOfMonth(
      dateFns.subMonths(
        new Date(
          dateFns.getYear(this._visibleDate),
          dateFns.getMonth(this._visibleDate)
        ),
        1
      )
    );
    const day = Math.min(
      dateFns.getDaysInMonth(prevMonth),
      dateFns.getDate(this._visibleDate)
    );
    this._visibleDate = this.min
      ? dateFns.max(dateFns.setDate(prevMonth, day), this.min)
      : dateFns.setDate(prevMonth, day);
  ...
  onNextDatePicker(e) {
    ...
    const day = Math.min(
      dateFns.getDaysInMonth(nextMonth),
      dateFns.getDate(this._visibleDate)
    );
    this._visibleDate = this.max
      ? dateFns.min(dateFns.setDate(nextMonth, day), this.max)
      : dateFns.setDate(nextMonth, day);
  ...
  onSelectYearDatePicker(e) {
    ...
    if (this.options.enableYearSwitch) {
      ...
      const currentYear = this._ui.body.years.querySelector(
  ...
  enableDate(date = undefined) {
    const index = this.disabledDates.findIndex((disableDate) =>
      dateFns.isEqual(disableDate, date)
    );
    if (index > -1) {
  ...
  highlightDate(date = undefined) {
    const index = this.highlightedDates.findIndex((highlightDate) =>
      dateFns.isEqual(highlightDate, date)
    );
    if (index > -1) {
  ...
  enableWeekDay(day) {
    const index = this.disabledWeekDays.findIndex((disabledWeekDay) =>
      dateFns.isEqual(disabledWeekDay, day)
    );
    if (index > -1) {
  ...
  disableWeekDay(day) {
    const index = this.disabledWeekDays.findIndex((disabledWeekDay) =>
      dateFns.isEqual(disabledWeekDay, date)
    );
    if (index > -1) {
  ...
  refresh() {
    ...
    const monthLabels = new Array(12)
      .fill(dateFns.startOfYear(this._visibleDate))
      .map((d, i) =>
    ...
    const yearLabels = new Array(this.options.displayYearsCount * 2)
      .fill(dateFns.subYears(this._visibleDate, this.options.displayYearsCount))
      .map((d, i) =>
    ...
    if (
      this.min &&
      dateFns.differenceInMonths(this._visibleDate, this.min) === 0
    ) {
    ...
    if (
      this.max &&
      dateFns.differenceInMonths(this._visibleDate, this.max) === 0
    ) {
    ...
    this._ui.navigation.month.innerHTML = dateFns.format(
      this._visibleDate,
    ...
    this._ui.navigation.year.innerHTML = dateFns.format(
      this._visibleDate,
      'YYYY',
      {
        locale: this.locale,
  ...
  _init() {
    ...
    this.highlightedDates = Array.isArray(this.options.highlightedDates)
      ? this.options.highlightedDates
      : [];
    for (var i = 0; i < this.highlightedDates.length; i++) {
      this.highlightedDates[i] = dateFns.format(
        this.highlightedDates[i],
        this.format,
        {
          locale: this.locale,
        }
  ...
  _bindEvents() {
    ...
    if (this._ui.navigation.previous) {
      this._clickEvents.forEach((clickEvent) => {
        this._ui.navigation.previous.addEventListener(
          clickEvent,
    ...
    if (this._ui.navigation.next) {
      this._clickEvents.forEach((clickEvent) => {
        this._ui.navigation.next.addEventListener(
          clickEvent,
          this.onNextDatePicker
        );
      });
    }
    ...
    if (this._ui.navigation.month) {
      this._clickEvents.forEach((clickEvent) => {
        this._ui.navigation.month.addEventListener(
          clickEvent,
    ...
    if (this._ui.navigation.year) {
      this._clickEvents.forEach((clickEvent) => {
        this._ui.navigation.year.addEventListener(
          clickEvent,
          this.onSelectYearDatePicker
        );
      });
    }
  ...
  _bindDaysEvents() {
    [].forEach.call(this._ui.days, (day) => {
      this._clickEvents.forEach((clickEvent) => {
        ...
        const onClick = !this._isValidDate(
          new Date(day.dataset.date),
          this.min,
          this.max
        )
          ? null
          : this.onDateClickDatePicker;
  ...
  _renderDays() {
      ...
      .map((s, i) => {
        ...
        const isInRange =
          ...
          dateFns.isWithinRange(
            theDate,
            dateFns.startOfDay(this.start),
            dateFns.endOfDay(this.end)
          );
        let isDisabled = this.max
          ? dateFns.isAfter(
              dateFns.startOfDay(theDate),
              dateFns.endOfDay(this.max)
            )
          : false;
        isDisabled =
          !isDisabled && this.min
    ...
    this._ui.body.dates.appendChild(
      document.createRange().createContextualFragment(templateDays(days))
    );
  ...
  _select(date = undefined) {
    ...
    if (
      ...
      (!this._isValidDate(this.start) ||
        (this._isValidDate(this.start) && this._isValidDate(this.end)))
    ) {
      this.start = date;
      this.end = undefined;
    ...
    if (
      ...
      new Array(dateFns.differenceInDays(this.end, this.start) + 1)
        ...
        .map((s, i) => {
          ...
          const dateElement = this._ui.body.dates.querySelector(
            `.datepicker-date[data-date="${theDate.toString()}"]`
          );
          if (dateElement) {
